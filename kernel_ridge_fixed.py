...
def run_permutation_test(X_train_svd, y_train, X_val_svd, y_val, alpha, kernel, gamma, random_seed):
    ...
    krr_perm = KernelRidge(alpha=alpha, kernel=kernel, gamma=gamma)
    krr_perm.fit(X_train_svd, y_train_perm)
    y_pred_perm = krr_perm.predict(X_val_svd)
...
for fold, (train_idx, val_idx) in enumerate(gkf.split(X, y, groups=groups), start=1):
    ...
    X_train_svd = svd.fit_transform(X_train_fold)
    X_val_svd = svd.transform(X_val_fold)
    print("SVD done.")

    # --------------------------------------------------------
    # Kernel Ridge fit on TRAIN only
    # --------------------------------------------------------
    print(f"Fitting KernelRidge(alpha={MODEL_ALPHA}, kernel={KERNEL}, gamma={GAMMA})...")
    krr = KernelRidge(alpha=MODEL_ALPHA, kernel=KERNEL, gamma=GAMMA)
    krr.fit(X_train_svd, y_train)
    y_pred = krr.predict(X_val_svd)
...
print("\n" + "=" * 60)
print("CROSS-VALIDATION SUMMARY — Kernel Ridge + SVD")
print("=" * 60)
