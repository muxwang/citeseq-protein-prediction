...
def run_permutation_test(X_train, y_train, X_val, y_val, random_seed):
    ...
    ridge_perm.fit(X_train, y_train_perm)
    y_pred_perm = ridge_perm.predict(X_val)
    return compute_score(y_val, y_pred_perm)


def encode_cell_types(cell_type_array, known_types):
    # Known types -> one-hot encoding
    # 'hidden' cells -> all zeros (unknown type)
    n_cells = len(cell_type_array)
    n_types = len(known_types)
    encoding = np.zeros((n_cells, n_types), dtype=np.float32)
    for j, ct in enumerate(known_types):
        encoding[:, j] = (cell_type_array == ct).astype(np.float32)
    return encoding
...
cell_types = metadata_filtered["cell_type"].to_numpy()
...
print("\nRemoving corrupted donor 27678 cells...")
...
# Known cell types for one-hot encoding
# HSC=Hematopoietic Stem Cell, NeuP=Neutrophil Progenitor
# EryP=Erythrocyte Progenitor, MasP=Mast Cell Progenitor
# MkP=Megakaryocyte Progenitor, MoP=Monocyte Progenitor
# BP=B-Cell Progenitor, hidden=unknown/withheld (all zeros)
known_types = ["HSC", "NeuP", "EryP", "MasP", "MkP", "MoP", "BP"]
print(f"\nKnown cell types: {known_types}")
print("'hidden' cells will receive all-zero encoding")
...
cell_types_sampled = cell_types[sample_idx]

print(f"Sampled {len(sample_idx)} cells")
print("Sampled cell type distribution:")
unique, counts = np.unique(cell_types_sampled, return_counts=True)
...
for fold, (train_idx, val_idx) in enumerate(gkf.split(X, y, groups=groups), start=1):
    ...
    ct_train = cell_types_sampled[train_idx]
    ct_val = cell_types_sampled[val_idx]
    ...
    # HVG selection on TRAIN fold only
    
    print(f"Selecting top {USE_TOP_GENES} HVGs...")
    ...
    # Add cell type one-hot encoding
    # known types -> one-hot, 'hidden' -> all zeros
    
    ct_train_encoded = encode_cell_types(ct_train, known_types)
    ct_val_encoded = encode_cell_types(ct_val, known_types)
    X_train_final = np.hstack([X_train_svd, ct_train_encoded])
    X_val_final = np.hstack([X_val_svd, ct_val_encoded])
    print(f"Feature matrix shape with cell type: {X_train_final.shape}")
    ...
    y_pred = ridge.predict(X_val_final)
...
print("\n" + "=" * 60)
print("CROSS-VALIDATION SUMMARY — Ridge + SVD + HVG + Cell Type")
print("=" * 60)
...
print(f"\nFor comparison:")
print(f"Ridge + SVD + HVG (no cell type): 0.882085")
print(f"Ridge + SVD + HVG + Cell Type:    {np.mean(fold_scores):.6f}")
print(f"Difference: {np.mean(fold_scores) - 0.882085:.6f}")
