...
days_train = days[train_mask]
days_val = days[val_mask]
...
# 7. Add day as explicit temporal feature
# For validation set, day=4 for all cells

train_days_col = days_train.reshape(-1, 1)
val_days_col = days_val.reshape(-1, 1)
X_train_final = np.hstack([X_train_svd, train_days_col])
X_val_final = np.hstack([X_val_svd, val_days_col])
print(f"Feature matrix shape with day: {X_train_final.shape}")


# 8. Ridge on training days only
...
print("\n" + "=" * 60)
print("TEMPORAL HOLDOUT — Ridge + SVD + HVG + Day Feature")
...
print(f"\nComparison:")
print(f"Temporal holdout without day feature: 0.872143")
print(f"Temporal holdout with day feature:    {model_score:.6f}")
print(f"Difference: {model_score - 0.872143:.6f}")
