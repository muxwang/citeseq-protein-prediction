...
for fold, (train_idx, val_idx) in enumerate(gkf.split(X, y, groups=groups), start=1):
    ...
    # Add day as explicit temporal feature
    # Appended as additional column after SVD components
    
    train_days_fold = days_sampled[train_idx].reshape(-1, 1)
    val_days_fold = days_sampled[val_idx].reshape(-1, 1)
    X_train_final = np.hstack([X_train_svd, train_days_fold])
    X_val_final = np.hstack([X_val_svd, val_days_fold])
...
print("\n" + "=" * 60)
print("CROSS-VALIDATION SUMMARY — Ridge + SVD + HVG + Day Feature")
print("=" * 60)
