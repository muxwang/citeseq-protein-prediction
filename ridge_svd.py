...
def run_permutation_test(X_train, y_train, X_val, y_val, n_components, random_seed):
    y_train_perm = y_train.copy()
    rng = np.random.default_rng(random_seed)
    for j in range(y_train_perm.shape[1]):
        rng.shuffle(y_train_perm[:, j])

    svd_perm = TruncatedSVD(n_components=n_components, random_state=random_seed)
    X_train_svd = svd_perm.fit_transform(X_train)
    X_val_svd = svd_perm.transform(X_val)
...
for day in valid_days:
    ...
    n_sample_day = max(1, min(n_sample_day, n_avail))
...
# 6. Build grouped CV labels — identical to PLS script
...
for fold, (train_idx, val_idx) in enumerate(gkf.split(X, y, groups=groups), start=1):
    ...
    # Gene selection on train fold only
    ...
    print(f"Fitting TruncatedSVD with n_components={n_comp_fold}...")
...
print("\n" + "=" * 60)
print("CROSS-VALIDATION SUMMARY — Ridge + SVD")
print("=" * 60)
